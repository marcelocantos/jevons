...
## Interaction — the only interface is Jevons

The product ships with essentially **no UI**: a conversation with Jevons,
and nothing else — no menus, dashboards, forms, or panels that we build
and maintain. This is not a bare interface; it means **all UI is
generated by Jevons on demand**, the same way it generates a chart or a
diagram (the generative visual surface — see 🎯T29). Even *interactive*
controls — a set of cards you drag to reprioritise, a diff with
approve/reject — are conjured in the moment when they serve the
exchange, and their actions flow back as intent (→ Jevons → doit), never
mutating state directly (the presentation-only bridge).

The distinction is load-bearing: **pre-built UI is banned; generated UI
is the whole point.** Don't build UI — build the agent that builds UI.
Any surface we find ourselves hand-crafting as a product feature is a
smell that the generative path isn't being trusted.
