...
func main() {
	...
	providerFlag := flag.String("provider", "", "default agent backend (grok, claude, â¦; empty = config.yaml / JEVONS_PROVIDER / grok)")
	...
	// of running as their own â a directed turn then never submits.
	...
	// ð¯T282: and reconcile the claudia tmux server, which hands each new
	// agent window the environment of whatever started the server â
	...
	// Structured config (ð¯T44): built-in defaults â config.yaml â flags.
	cfgPath := *configPath
	if cfgPath == "" {
	...
	// before launch (ensureOverseerMCPServer: Grok â ~/.grok/config.toml,
	// Claude â claude mcp -s user; ð¯T58 + ð¯T212). That attaches on both
	// session/new and session/load â so we no longer pass a session-scoped
	...
	// loopback-only default (ð¯T6) binds IPv4 127.0.0.1, and localhost may
	// resolve to ::1.
	...
	// and replayed to reconnecting clients â no conversation is ever lost.
	...
	if xaiKey != "" {
		...
		slog.Info("no xAI API key â voice bridge disabled (set XAI_API_KEY or store via: security add-generic-password -a jevons -s xai-api-key -w YOUR_KEY)")
	}

	// ð¯T8.2 worker observability (SQLite workers/events + SSE hub).
	workerTracker, err := workers.NewTracker(filepath.Join(cfg.StateDir, "workers.db"))
	if err != nil {
		slog.Error("workers tracker unavailable â jwork observability disabled", "err", err)
		workerTracker = nil
	} else {
	...
	// ð¯T27.2: structured provider config + persistence home for the registry.
	// Desired set comes from config.yaml `providers:`; runtime state lives in
	// StateDir/providers/registry.db. Reconcile/supervisor is ð¯T27.3.
	...
	if pstore, err := provider.OpenStore(provider.DefaultStorePath(cfg.StateDir)); err != nil {
		slog.Error("provider store unavailable â external providers disabled", "err", err)
	} else {
		defer pstore.Close()
		...
		if err != nil {
			slog.Error("provider config load failed â external providers disabled", "err", err)
		} else {
			providerCfg = pm
	...
	// providerCfg feeds the ð¯T27.3/T27.4 lifecycle + aggregator wiring
	...
	if eng, err := doit.Open(doit.OpenArgs{
		...
		slog.Error("doit engine unavailable â jwork policy gate disabled", "err", err)
	} else {
		doitEng = eng
		defer eng.Close()
	...
	// ð¯T27.3/ð¯T27.4/ð¯T27.5: provider supervisor + MCP tool aggregation +
	...
	if providerCfg != nil {
		...
		feedReg.HubID = "jevons" // loop-safety (Â§5.4): drop self-origin relays
		feedHub := provider.NewFeedHub(provider.FeedHubArgs{
			...
			// ð¯T27.6: recompose and push the provider view on UI-relevant
		...
		// ð¯T27.9: automation liveness â declared automations are checked
	...
	// ð¯T335: writ confinement substrate (CLI) â seatbelt + egress for high-risk exec.
	...
	if writAvail {
		...
		slog.Warn("writ binary not found â confined-exec uses pure allowlist only until writ is on PATH")
	}
	...
	// ð¯T40: SIGINT/SIGTERM = normal stop (StopAll); SIGHUP = upgrade exit
	...
	upgradeSnap, err := upgrade.LoadSnapshot(upgrade.SnapshotPath(cfg.StateDir))
	if err != nil {
	...
	if snap := upgradeSnap; snap != nil {
		...
		// Launch/Adopt reattaches to still-running serve processes.
	...
	// Token-spend clamp-down (ð¯T36): start it before the butler so its
	// spawn/resume guards feed the butler's lifecycle. Returns nil if the
	// usage DB is unavailable â the cockpit still runs, just unguarded.
	guard := startCostGuard(ctx, cfg, registry, scanner, srv)

	// ð¯T114: same Claudia adapter is Fleet (threads) and Participants
	...
	// loser when it disagrees with the owner-visible default.
	portfolioPath := filepath.Join(cfg.StateDir, "llm-portfolio.json")
	if pf, loaded, err := cost.LoadPortfolioOverride(portfolioPath); err != nil {
		slog.Error("llm portfolio override unreadable — using compiled seed",
			"path", portfolioPath, "err", err)
	} else if loaded {
		mcpSrv.SetLLMPortfolioSource(pf, true)
		if note := cost.DescribeConfigPortfolioDisagreement(string(defaultProvider), pf, true); note != "" {
			slog.Warn("llm portfolio override disagrees with config.yaml — omit-provider mint follows config",
				...
				"portfolio_default", pf.DefaultProvider,
				"portfolio_work_mint", cost.WorkMintPortfolioProvider(pf),
				"cite", note)
		} else {
	...
	// ð¯T325.2: multi-provider portfolio soft-cap overlays from budget.json
	...
	// ð¯T243 ambient RSI coach (product path): judgments â overseer.
	...
	// ð¯T357 periodic full-scan audit: bounded advanced-model passes over
	...
	// ð¯T148: keep stored provider on resume; only set default when empty.
	jevonDef.Provider = cli.SelectAgentProvider("", jevonDef.Provider, defaultProvider)
	// ð¯T114: overseer purpose on the unified participant record.
	if jevonDef.Purpose == "" {
		jevonDef.Purpose = claudia.PurposeOverseer
	}

	// Overseer MCP registration happens after the listener is bound, so the
	// advertised URL names the port actually served (ð¯T379). See below.
	...
	// ð¯T82: event-driven fleet panel refresh when agents.json mutates.
	...
	// ð¯T385: the cross-site guard wraps the whole mux, so route groups
	// mounted outside internal/server â the MCP endpoint (which exposes
	// jevons_writ_exec) and the dev server's static routes â are covered on
	...
	// provider (ð¯T58 Grok, ð¯T212 Claude), registered before launch so tools
	...
	//
	// ð¯T379: this must come *after* the bind, and must advertise
	// servedPort(ln) rather than cfg.Port. Registering first meant that a
	// daemon which then failed to listen â the port already held by another
	// instance â exited having just written a registration for an endpoint
	...
	defer func() {
		...
		if mode.StopAgents() {
			registry.StopAll()
			if err := upgrade.ConsumeSnapshot(upgrade.SnapshotPath(cfg.StateDir)); err != nil {
				slog.Warn("could not drop stale upgrade handoff on drain", "err", err)
			}
			return
		}
	...
	if jevonProc := registry.Get(cfg.OverseerName); jevonProc != nil {
		...
		// (ð¯T285). No-op when nothing is pending.
		...
		// ð¯T118: ACP progress / tool steps â RHS fleet-row status chrome.
		// agents_changed only when the glanceable summary changes.
		// ð¯T209: same hook multiplexes inspect live frames on /ws/chat for
		...
		// (ð¯T61): unlike MCP-tool spawns, they never passed through
		...
		// ð¯T58), so the model already has full context. The recap-on-boot
		// hack â a lossy summary re-injected as a visible user turn every
		// restart â is gone.
	} else {
	...
	// ð¯T335: standing security auditor â always on; deliver via overseer when attached.
	...
	// ð¯T317 impatience ladder (T316 set + T318 attenuation + sinks).
	// Constructed at the SetIdlePressureHooks seam (2a065e5): RePressure â
	// idle-nudge deliver, Overseer â event_push to root, Human â sticky OS
	...
	if be, err := sticky.NewOSNotifier(); err != nil {
		slog.Warn("impatience sticky unwired â human rung will error until terminal-notifier is available",
			"err", err)
	} else if st := sticky.New(be); st != nil {
		humanSink = st
		// Withdraw every raised alert on shutdown so a stale "stuck" does not
		// outlive the daemon that could have cleared it (ð¯T319).
		defer func() {
	...
	// ð¯T415: the owner-notice path writes straight to the owner's journal.
	...
	// 🎯T415.1: the diagnostician runs DETACHED, so it survives the
	// daemon and can restart it without dying. Empty path leaves diagnosis
	...
	// ð¯T254.1: unattended frontier consumption â every non-design-gated ready
	...
	if cfg.FrontierConsume.Disabled {
		slog.Info("frontier consume loop disabled by config (ð¯T254.1)")
	} else {
	...
	// ð¯T50/ð¯T54 regression oracle, hoisted out of the attach block so it
	...
	go func() {
		...
		if mcpSrv.ToolsListCount() == 0 {
			slog.Error("OVERSEER TOOLLESS OR ABSENT â no MCP tools/list since boot; jevons tools are not attached (see ð¯T50/ð¯T54)")
			srv.Broadcast(map[string]any{
				"type":  "error",
	...
	if *relayURL != "" {
		...
		slog.Info("no credential and no relay configured â pair a device with `jevonsd --pair <instance-id> --relay <url>`")
	}
...
// overseerMCPKind names which CLI path installs overseer MCP tools (ð¯T212).
...
// overseerMCPKindForProvider is the pure oracle for ð¯T212: which user-scoped
// MCP ensure path the boot/attach code must take. Default/empty and Grok â
// Grok config.toml; Claude â claude mcp -s user; Codex/Bedrock â none
...
// overseer provider before launch (ð¯T58, ð¯T212). Grok writes user-scoped
...
// session/load â the key to the overseer resuming its real session across
...
// Claude overseer gets jevons tools on session start and resume (ð¯T212).
...
// mount) and the host a concrete address â never "localhost" (::1 vs
// 127.0.0.1; ð¯T6/ð¯T58). Shared by Grok and Claude ensure paths (ð¯T212).
//
// port is the port the daemon actually serves, not the configured one
// (ð¯T379). Those differ whenever the listen fails or the config asks for an
// ephemeral port, and a registration written from the intended value in
// either case advertises an endpoint no process is behind â which is the
...
// (ð¯T212 oracle: user scope so tools survive resume; transport http).
...
// diagnoseOverseerUnavailable is the pure oracle for ð¯T214 J4: message text
...
func diagnoseOverseerUnavailable(provider claudia.Provider, binOnPath bool, candidatePath string) string {
	...
	case claudia.ProviderClaude:
		if binOnPath {
			return "the Claude agent failed to start â check that the Claude CLI is authenticated " +
				"and see the jevonsd log for details (overseer provider is claude)"
		}
		return "the Claude CLI is not installed (or not on PATH) â install Claude Code, authenticate, " +
			"then restart jevonsd (overseer provider is claude; this is not a Grok CLI issue)"
	case claudia.ProviderCodex:
		if binOnPath {
			return "the Codex agent failed to start â check Codex auth and see the jevonsd log " +
	...
	default:
		...
		if binOnPath {
			return "the Grok agent failed to start â check that the Grok CLI is signed in " +
				"(`grok login`, or set XAI_API_KEY) and see the jevonsd log for details"
		}
		if candidatePath != "" {
			return "the Grok agent failed to start â check that the Grok CLI at " + candidatePath +
				" is signed in (`grok login`, or set XAI_API_KEY)"
		}
		return "the Grok CLI is not installed â install Grok Build and sign in " +
...
// startAmbientRSICoach wires the ð¯T243 product path: drip-read surfaces â
// structured judgments â overseer (never bullseye file).
// Env:
//
//	JEVONS_RSI_COACH=0       â disable coach schedule (MCP tools still register if started)
//	JEVONS_RSI_COACH_INTERVAL â duration (default 15m); "0" disables ticker
//	JEVONS_RSI_NO_CHAT       â "1" skip owner-chatlog surface
//	JEVONS_RSI_NO_SESSION    â "1" skip session-transcript surface
func startAmbientRSICoach(ctx context.Context, cfg config.Config, mcpSrv *mcpserver.Server, gate capacity.Gate) *rsi.Coach {
	...
	// Retrospective history mine (ð¯T353): the drip starts at EOF, so past
...
// startAmbientResearch wires the ð¯T356 standing research cycle: periodic
...
// Env:
//
//	JEVONS_RESEARCH=0        â disable the schedule (MCP tools stay registered)
//	JEVONS_RESEARCH_INTERVAL â duration (default 90m); "0" disables the ticker
...
// startPeriodicAudit wires the ð¯T357 standing full-scan audit: a bounded,
...
// Env:
//
//	JEVONS_AUDIT=0         â disable the schedule (MCP tools stay registered)
//	JEVONS_AUDIT_INTERVAL  â duration (default 24h); "0" disables the ticker
func startPeriodicAudit(ctx context.Context, cfg config.Config, mcpSrv *mcpserver.Server, gate capacity.Gate) *audit.Auditor {
	...
	// inspectable and retunable rather than hardcoded (ð¯T357 acceptance #2).
...
// startAmbientRSIMint wires residual ð¯T92 direct bullseye mint (NOT product path).
// Product path is the coach (ð¯T243). Phrase-list / mint only when explicitly opted in:
//
//	JEVONS_RSI_MINT=1  â enable eventlog mint loop (still subject to DRY_RUN)
//	JEVONS_RSI_DEEPER=1 â also feed chatlog+session phrase extract into mint (noisy)
...
func startAmbientRSIMint(ctx context.Context, cfg config.Config, mcpSrv *mcpserver.Server) *rsi.Loop {
	...
	if !mintOn && !deeper {
		slog.Info("ambient RSI mint residual off (product path is coach ð¯T243); set JEVONS_RSI_MINT=1 to enable")
		return nil
	}
