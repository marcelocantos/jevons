...
| T191 restart-daily | `scripts/restart-daily-jevonsd.sh` exists, executable, `/bin/bash -n`, `--help`/`--dry-run`, nohup/setsid detach markers; persona/AGENTS/agents-guide carry T188+T191 doctrine. Not a live development bounce. |
| T194 / T552 development-surface | persona/AGENTS/agents-guide/fleet_brief: owner-visible claims are observed (T552); hermetics alone are not enough; `HasDailyPathEvidence` is a seam classifier, not an achieve gate. T553.2 workers do not auto-bounce. Pure classifier tests live in `internal/mcpserver`. Not a live development bounce. |
