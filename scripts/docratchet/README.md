...
| T536.1 silent-decision ledger | persona/AGENTS/agents-guide/fleet_brief require `silent-ledger none|ranked` on finish-reports; schema + `ReadSilentLedger` live in `internal/envelope`. Not a live fleet send. |
| T536.2 auditor role | `internal/roles/builtin/auditor.md` carries read-only + ledger-challenge doctrine; agents-guide names `role=auditor` + `internal/roles`; FleetStandingBrief must not hold if-you-are-auditor. Not a live spawn. |
| T536.3 fog-of-war scout | persona/AGENTS/agents-guide/fleet_brief require scout before implement (`phase scout`, `scout-report`, fog-known/unknown/blindspot, InheritLedger); skip rules still hold; schema in `internal/envelope`. Not a live fleet send. |
| T493.1 visual cockpit prose | persona/AGENTS/agents-guide/fleet_brief carry doctrine: after `#messages` / React `ui/` transcript visual work (🎯T540), a four-part prose look (ink / empty / Latest / normal transcript yes-or-no); `visibleInScroller` and screenshot-tool captions are not the verdict; automatic no; false-green journey same-turn oracle fix; `HasVisualProseVerdict` / `LooksLikeMissingVisualVerdict`. Pure classifier tests live in `internal/mcpserver`. Not a live screenshot. |
...
| T438 clean-checkout UI | The Playwright half of the same shape: a detached worktree of **HEAD** runs `make playwright-deps`, then `require(playwright)` succeeds and one UI oracle (`t370-fleet-cycle-test.js`) exits 0 — or skips naming a missing browser binary. Never `MODULE_NOT_FOUND` (the failure that forced T370 to symlink into the shared clone). `-short` covers install+require only. Not a doc grep — a real install in the committed tree. |
