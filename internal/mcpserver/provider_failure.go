...
// logProviderFailure emits the shared structured failure line and, when the
// refusal is a hard block, stamps fleet intent blocked_provider (🎯T406).
func (s *Server) logProviderFailure(surface, target string, class agenterr.Class, raw string) {
...
func (s *Server) toolFailure(surface, target string, err error) *mcp.CallToolResult {
	if err == nil {
		return nil
	}
	...
	s.logProviderFailure(surface, target, class, err.Error())
...
//
// A real reply also clears a provider hard-block (🎯T406 clause 4): evidence
// the provider accepts work again, not a timer and not an assertion.
func (s *Server) replyFailure(surface, target, reply string) *mcp.CallToolResult {
	class, ownerMsg, ok := agenterr.ReplyFailure(reply)
	if !ok {
		s.ObserveProviderOK()
		return nil
	}
	s.logProviderFailure(surface, target, class, reply)
	return mcp.NewToolResultError(ownerMsg)
}
