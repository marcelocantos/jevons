...
func (s *Server) registerAgentReportTools() {
	...
	s.mcpSrv.AddTool(
		mcp.NewTool("jevons_agent_report_read",
			mcp.WithDescription("Read an agent's stored turn report in full (🎯T388). Every report an agent delivers is stored before delivery, so this works AFTER the agent auto-deregisters on its terminal report — unlike jevons_agent_send, which answers \"agent is not running\". Use it when a delivered report carries the [⚠️ REPORT TRUNCATED] marker, or when you need one part of a report again: the bytes come from the store, so the agent never re-derives (and so cannot rewrite) what it already said. Omit report_id for the latest report; omit section for the whole text; pass list=true to see what is stored."),
