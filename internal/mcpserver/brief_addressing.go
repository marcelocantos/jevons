...
// Brief addressing (🎯T452).
//
// THE INCIDENT, 2026-08-15. A post-restart-brief composed for
// jv-t443-red-as-proof — ROLE: work agent, PARENT: jevons-po, TARGET: 🎯T443 —
// arrived as a queued_command in the OVERSEER's composer, in a window where two
// correctly addressed overseer briefs arrived alongside it. Every composition
...
// one you act on." Delivered into the wrong seat, the anti-confusion instrument
// becomes an instruction to impersonate — a name, a parent, a bound target and
// the role doctrine of a role the reader does not occupy. The overseer that
// caught this caught it by checking jevons_agent_list against its own session
// id, which is exactly the check the header promises is unnecessary.
//
// SO THE TEXT AND THE DESTINATION ARE BOUND. The header is not decoration the
// send path may ignore; it is an assertion about who is receiving this message,
// and the send path is where that assertion can still be checked. A payload
// whose header names an agent other than the seat it is about to enter is
// refused, loudly, with both names in the error — a misdelivery that fails is
// an incident someone can read, and a misdelivery that succeeds is an agent
// quietly working someone else's target.

// identityNameLabel is the line FormatIdentityHeader writes the recipient's
// durable name on.
const identityNameLabel = "- NAME: "
...
// header is the report this target asks a misdelivered-to agent to send, and
...
func IdentityHeaderName(text string) string {
	...
	for n, line := range lines {
		...
		if name, ok := strings.CutPrefix(line, identityNameLabel); ok {
...
// identity header naming an agent other than dest.
//
// A message with no header is not this target's business: relayed digests,
...
// is case-insensitive because deliverByName already resolves names that way.
