...
Built-in roles cannot be deleted — only overridden. A role with live
instances cannot be removed without `force=true`.
...
**Fast-cheap models (🎯T325.2.1):** omit-`model` mint with `task_type`
`mechanical` / `ops_classify` (or nudge/ack/small_edit) pins `grok-build`
on Grok or `gpt-5.3-codex-spark` on Codex. Explicit `model=` still wins.
Spark is not pinned onto a red Codex weekly (🎯T390.1.5). A prompt over
the model's context window (Spark 128k; grok-build 256k) escalates to the
provider frontier, cited as `model_knob: escalated`. Implementation /
overseer-CEO turns do not land on this class (do not pre-empt 🎯T358).
