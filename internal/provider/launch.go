...
// execProcess adapts *exec.Cmd to Process. Wait is guarded by a sync.Once
// so the runner goroutine and Stop can both call it — os/exec permits
// exactly one real Wait per process.
...
// sooner. It returns once the process is reaped, so a disabled provider
// never lingers as a zombie.
func (p *execProcess) Stop(ctx context.Context) error {
	if p.cmd.Process == nil {
		return nil
	}
	// Someone must be reaping for done to close; Wait is idempotent.
	go p.Wait()
